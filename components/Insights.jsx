...
function AnimatedInsightCard({ card, yOffset }) {
  return (
    <motion.div className="relative overflow-hidden bg-black w-full h-[400px]" style={{ y: yOffset }}>
      <Link href={card.link} className="block h-full">
        <div className="h-[240px] relative">
          <Image src={card.image || "/placeholder.svg"} alt={card.title} fill className="object-cover" />
        </div>
  ...
  return (
    <div className="bg-[#1e2124] text-white py-16 md:py-24" ref={containerRef}>
      ...
      >
        ...
        {isMobile && (
          <div className="overflow-x-auto -mx-4 px-4 pb-6" style={{ scrollbarWidth: "none", msOverflowStyle: "none" }}>
        ...
        {!isMobile && (
          <div className="hidden md:grid md:grid-cols-3 gap-8 perspective">
            {insightCards.map((card, index) => (
              <AnimatedInsightCard key={index} card={card} yOffset={cardOffsets[index]} />
            ))}
          </div>
        )}
      </div>
